...
export function renderEventStorming(
  ...
  nodesGroup
     ...
     .on('mouseenter', function (this: Element, event: MouseEvent) {
      ...
      // Show implicit error node generated by getOrCreateNegativeNode when no explicit altNext resolved.
      if (!flowParts.some((p) => p.startsWith('✕'))) {
        const implicitErrorId = `error_default_${node.id}`;
        const implicitError = model.nodes.find((n) => n.id === implicitErrorId);
        if (implicitError) {
          flowParts.push(`✕ ${escapeHtml(implicitError.label)}`);
        }
      }
